...
# 专题必须覆盖的核心方法清单（用于生成后完整性校验与自动补全）
# key 为专题名称中的关键词，value 为必须出现的方法要点（不带“法”字，避免措辞误判）
FINE_TOPIC_METHODS = {
    "数列": ["累加", "累乘", "错位相减", "裂项", "倒序相加", "分组求和", "并项", "构造", "数学归纳", "放缩", "取倒数", "待定系数"],
    "不等式": ["基本不等式", "均值不等式", "比较法", "综合法", "分析法", "放缩", "换元", "判别式", "数形结合", "配方法"],
    "函数": ["定义域", "值域", "单调性", "奇偶性", "周期性", "对称性", "零点", "图像变换", "换元", "分离参数"],
    "三角函数": ["诱导公式", "和差角", "二倍角", "辅助角", "正弦定理", "余弦定理", "图像变换", "合一变形"],
    "导数": ["导数的几何意义", "单调性", "极值", "最值", "参数讨论", "构造函数", "放缩", "零点"],
    "解析几何": ["直线与圆", "椭圆", "双曲线", "抛物线", "弦长", "中点弦", "韦达定理", "判别式", "离心率"],
    "立体几何": ["线面平行", "线面垂直", "面面垂直", "异面直线", "二面角", "体积", "空间向量", "截面"],
    "概率统计": ["古典概型", "几何概型", "条件概率", "期望", "方差", "二项分布", "正态分布", "回归"],
    "集合": ["并集", "交集", "补集", "子集", "真子集", "空集", "描述法"],
    "复数": ["复数的模", "共轭复数", "复平面", "纯虚数", "实部", "虚部"],
    "向量": ["数量积", "模长", "夹角", "垂直", "平行", "基底", "投影"],
    "圆锥曲线": ["焦点", "准线", "通径", "弦长", "韦达定理", "离心率"],
}
...
【板块细化要求 - 必须严格遵守】
1. 【核心概念与定义】：本专题所有基础概念都要讲到，不只列定义，还要给概念辨析与易混点（如数列与函数的关系、等差与等比区别、通项与前n项和关系等），至少 6 条；每条给一句话说明或示例。
2. 【必背公式/定理/定律】：按类全量罗列，本专题至少 10 条，每条格式为“【名称】公式（适用条件或易错提醒）”；不得与板块1核心概念重复（板块1只写定义，这里只写公式）。
   【方法完整性硬性要求】必须覆盖本专题全部核心方法与公式，尤其是求通项与求和的所有方法，一个都不能少：
   - 求通项：公式法、累加法、累乘法、构造法（待定系数/特征根）、取倒数法、取对数法、换元法；
   - 求和：公式法、倒序相加法、错位相减法、裂项相消法、分组求和法、并项法。
   这些方法的适用形式（如“aₙ₊₁-aₙ=f(n)→累加”“aₙ₊₁/aₙ=f(n)→累乘”）与操作要点必须体现在公式或方法清单中，禁止遗漏任何一条。
   可在末尾设“秒杀结论”小节，只保留严格可证、有普适性的结论并标注适用条件，给不出严谨结论的宁可不写。
3. 【基本性质与规律】：按“等差类/等比类/通用类（含前n项和Sn与an关系、数列最值、周期性、奇偶项、倒序相加特征等）”分类罗列，本专题至少 8 条；每条给“表述 + 证明思路或典型用法”。
4. 【题型板块（4~7）】：内部统一按“总—分—难度”路线组织：先列本板块覆盖的方法总览（总），再对每个方法给“方法要点1~2句 + 例题（按基础→中档→综合难度递进）”（分）。
   【方法完整性硬性要求】B级（核心方法）必须覆盖本专题全部核心方法，一个都不能少。数列专题至少包括：累加法、累乘法、构造法（含待定系数/特征根）、取倒数法、错位相减法、裂项相消法、倒序相加法、分组求和法、并项法，每个方法都要配至少 1 道完整例题（含题面+分步解析+答案），宁可内容长也不许漏方法。
   各题型板块之间不重叠：A级只放最基础方法，B级放核心方法并逐一展开，C级放多方法综合，D级放高考真题综合。同一道题不要重复出现在不同板块。
5. 【题型识别特征速查】：每条统一写成“看到【具体特征】→【首选方法】，例如：…”；按“求通项类/求和类/证明类/综合类”分组，至少 12 条；特征必须具体可判断（禁止“出现等差等比字眼”这类空泛表述）。
6. 【典型易错点诊断】：至少 6 条，每条固定三段式——【易错点】…【典型错误】…（含一段错误示范与错误结论）【正确做法】…（含正确结论）；每条必须结合本专题具体题目展开。
7. 【重难点突破】：只聚焦本专题 2~3 个真正难啃的核心方法，不贪多；每个方法固定四步——适用类型、标准步骤（编号）、完整例题（含思路+分步解析+最终答案）、常见卡点提醒；禁止重复出现同名方法、禁止编号混乱。
8. 【真题部分（D级与板块12）】：所有真题/改编题必须给出完整分步解析与最终答案。硬性红线：禁止出现“证明略”“易证”“显然”“同理”“过程略”“解答略”“步骤略”“因篇幅限制”“暂未展开”“此处省略”“待补充”“后续补充”“以下省略”“内容略”等省略性、占位性表述；绝不允许用“篇幅限制暂不展开”之类的话搪塞——写不完也要写出来，宁可写长不可省略，每个板块都必须给出真实完整内容。

【内容要求】
1. 内容准确、专业、详实，面向高中{subject}复习与备考场景；整体要像一份可直接上课使用的完整讲义，干货足、不空洞、不敷衍；
2. 核心概念、公式、性质等知识板块要全面覆盖，宁多勿少；例题解析要详细，先讲“为什么这样想”（思路）再给步骤，不机械罗列；
3. 题型部分每道题都必须给出【完整题面 + 分步解析 + 最终答案】，中档题（B/C级）要具体详细；
4. 拔高题（D级）以历年高考/会考真题题型为主，题目标注考法与年份（如“2023全国卷”），并给出贴近高考难度的完整解析；
5. 每个板块用「【板块名】」开头，直接输出正文，不要额外解释；
6. 总字数 8000~12000 字，保证内容丰富扎实、干货足，可直接用于备课或学生复习。
...
【板块细化要求】必背公式按类全量罗列至少10条（每条含名称/公式/适用条件），必须覆盖本专题全部求通项与求和的方法（如累加/累乘/构造/取倒数/错位相减/裂项/倒序相加/分组/并项），一个都不能少；基本性质分专题至少8条（每条含表述+用法）；题型板块内部按“方法总览→各方法要点+按难度例题”组织，B级核心方法必须覆盖本专题全部核心方法、每个方法至少1道例题；题型识别写成“看到【特征】→【方法】”并按求通项/求和/证明/综合分类至少12条；易错点每条用【易错点】【典型错误】【正确做法】三段式至少6条；重难点只聚焦2~3个核心方法（适用类型→标准步骤→完整例题→卡点提醒）。【硬性红线】禁止出现“证明略”“易证”“显然”“同理”“过程略”“解答略”“步骤略”“因篇幅限制”“暂未展开”“此处省略”“待补充”“后续补充”“以下省略”“内容略”等省略性、占位性表述；绝不允许用“篇幅限制暂不展开”之类的话搪塞——写不完也要写出来，宁可写长不可省略，每个板块都必须给出真实完整内容。
...
def generate_fine_content(subject="数学", topic="函数单调性", model_choice="auto", progress_cb=None):
        ...
        for attempt in range(2):
            try:
                ...
                if len(text) < min_chars and attempt == 0:
                    ...
                    prompt = prompt + f"\n\n【注意】你上一次输出太简略（仅 {len(text)} 字），请务必把本次的 4 个板块写充实：知识板块宁多勿少、全面覆盖；每道题给出完整题面+分步解析+最终答案，每个板块都要有实质内容，本次合计至少 2500 字，内容要像完整讲义一样扎实，不要偷懒。"
                    continue
                break
            except Exception as e:
                if attempt == 0:
    ...
    # ===== 方法完整性兜底补全 =====
    # 按专题配置的必须方法清单检查，缺失的方法自动生成一段补进 B级 核心题型板块
    topic_key = None
    for _k in FINE_TOPIC_METHODS:
        if _k in topic:
            topic_key = _k
            break
    if topic_key and md:
        must_have = FINE_TOPIC_METHODS[topic_key]
        missing = [m for m in must_have if m not in md]
        if missing:
            cb(f"[{topic}] 检测到缺失方法：{'、'.join(missing)}，正在自动补全...")
            patch_prompt = (
                f"你正在编写「{subject} · {topic}」专题的高中数学精细化精讲讲义。"
                f"现在讲义缺少以下核心方法的讲解，请逐个补齐（每个方法写一小节，约80~150字）：\n"
                f"{'、'.join(missing)}\n\n"
                f"每个方法按如下格式输出：\n"
                f"**方法名**\n"
                f"- 适用形式：看到什么特征用它（一句话）\n"
                f"- 操作要点：2~3步\n"
                f"- 例题：1道简短例题，含题面+关键步骤+最终答案\n\n"
                f"直接输出正文，内容要专业准确，不要多余解释。"
            )
            try:
                resp = llm_obj.invoke(patch_prompt)
                patch_text = resp.content if hasattr(resp, "content") else str(resp)
                patch_text = patch_text.strip()
                if patch_text:
                    inserted = False
                    for mi, seg in enumerate(merged):
                        if seg.startswith("【5."):
                            merged[mi] = seg + "\n\n**本专题核心方法补全（完整性保障）**\n\n" + patch_text
                            inserted = True
                            break
                    if not inserted:
                        merged.append(f"【5. B级核心题型】\n\n**本专题核心方法补全（完整性保障）**\n\n{patch_text}")
                    md = "\n\n".join(merged)
                    cb(f"[{topic}] 已补全 {len(missing)} 个缺失方法")
            except Exception as e:
                cb(f"[{topic}] 方法补全失败（不影响主文档）：{str(e)}")

    # ===== 占位/省略表述后置校验 =====
    placeholders = ["因篇幅限制", "暂未展开", "此处省略", "待补充", "后续补充",
                    "以下省略", "内容略", "篇幅有限", "不再赘述", "不展开",
                    "证明略", "过程略", "解答略", "步骤略", "易证", "同理"]
    hits = [p for p in placeholders if p in md]
    if hits:
        cb(f"[{topic}] ⚠️ 检测到占位/省略表述：{'、'.join(hits)}——模型存在偷懒内容，建议重新生成或人工补写")
